...
Muons created in the upper atmosphere have a half-life time $t_{1/2}' = 1.5\,\mu s$ in their rest frame $S'$. Their rest frame moves at $u = 0.99c$ relative to the Earth frame $S$. These cosmic muons are created at a height of $60$ km in the atmosphere. In a Galilean Universe, we would argue that it would take
\be
\Delta t = \Delta h / u = 60\;{\rm km}/ (0.99 c) \sim 200 \mu{\rm s}
\ee
...
\be
\gamma_u(u = 0.99c) \simeq 7 \rightarrow t_{1/2} \simeq 10.5\;\mu{\rm s}.
\ee
...
\begin{equation}
L = L_0 \sqrt{1 - u^2/c^2} = \frac{L_0}{\gamma_u}.
\end{equation}
Since $\gamma_u \geq 1$, $0\leq \gamma_u^{-1}\leq 1$, i.e., the length is contracted. Only for a (hypothetical) object that would move at $c$, the length would be contracted to zero.

\subsection*{Example 2: High-Speed Train}
A train of proper length $L_0 = 500$ m moves at $u = 0.8c$ relative to the ground frame. Find its length in the ground frame.

\begin{align*}
\gamma_u &= \frac{1}{\sqrt{1 - 0.64}} = \frac{1}{0.6} \approx 1.667 \\
...
\begin{tikzpicture}[scale=0.92,>=Stealth]
  ...
  \draw[->] (0,-0.25) -- (0,3.5) node[above] {$t$};
  \draw[orange,thick] (-2.7,2.7) -- (0,0) -- (2.7,2.7);
  \draw[blue,very thick,->] (0,0) -- (1.15,3.25) node[above] {$t'$};
  \draw[blue,very thick,->] (0,0) -- (3.25,1.15) node[right] {$x'$};
  \node[font=\small] at (2.55,2.9) {light};
  \node[font=\small] at (-2.55,2.9) {light};
  \node[font=\small,blue] at (1.55,2.85) {$x'=0$};
  \node[font=\small,blue] at (2.85,1.55) {$t'=0$};
\end{tikzpicture}
\caption{A qualitative two-observer diagram in units with $c=1$. The primed axes are a coordinate grid for the moving frame; no tick marks or interval construction are needed yet.}
\label{fig:two-observer-diagram}
\end{figure}

\begin{itemize}
    \item The $t'$ axis is the worldline of the origin of $S'$. If $S'$ moves with speed $u$ in $S$, then this line satisfies $x=ut$.
...
The important lesson is that a Lorentz transformation is not a rotation in the Euclidean sense. It changes the coordinate grid while preserving the physical facts established so far: inertial motion remains inertial, and light has the same speed in every inertial frame. In Lecture 4, we will identify the precise spacetime quantity that is unchanged. This picture should be kept next to the algebra below: the equations tell us how to compute coordinates, while the diagram tells us what the computation means.
...
\ee
Our goal will be to determine the constants $A,B,C$, and $D$. Like Galilean transformations, these functions will depend on $u$, the speed of frame $S'$ as measured in frame $S$. We reserve $v$ and $v'$ for the speed of an object measured in $S$ and $S'$, respectively.
...
Now things will get a little bit trickier. Suppose I am an observer in frame $S$ and I am not moving ($\Delta x = 0$). What is my velocity in the other frame? That is easy, it should be $-u$ (since I am at rest and the other frame $S'$ has speed $u$). The velocity in that other frame should be determined via $\Delta x'/\Delta t' = -u$. This implies that $B/A = u$ or $B = \gamma u$.
...
% Text Node
\draw (394,75) node [anchor=north west][inner sep=0.75pt]    {$u $};
% Text Node
...
\end{tikzpicture}
\caption{A train of length $L'$ (as measured at rest in frame $S'$) is moving to the right with speed $u$. A source on the train is emitting two pulses. They arrive at the rear and front of the train at the same time, as observed (by observer `A') in the rest frame $S$. Figure taken from R2, 2023-2024. }
\label{fig:rca}
\end{center}
\end{figure}

Consider the setup as shown in Fig.~\ref{fig:rca}, where a train is moving with speed $u$. The train has length $L'$ (as measured at rest -- proper length).
Two light rays/pulses are emitted to the rear and front of the train, which arrive simultaneously for observer A ($\Delta t = 0$). Light rays appear to move $c+u$ to the back of the train and $c-u$ to the front of the train. This is ok, since it is really the distance between the location of the emitted pulse and the front and rear of the train that is changing; the rear is moving towards this point with speed $u$, while the front is moving away from this point with speed $u$.
...
\be
\frac{D'_{\rm back}}{D'_{\rm front}} = \frac{c+u}{c-u} = \frac{1+\beta}{1-\beta},
\ee
where I have divided by $c$ and defined $\beta = u/c$ for this boost.
...
\ee
This allows us to find that $D/C = u/c^2$ and hence $D = \gamma u/c^2$.
The Lorentz transformations are thus given by:
\begin{tcolorbox}[colback=blue!5!white]
\vspace{-1.3em}
\begin{align}
\Delta x &= \gamma (\Delta x' + u \Delta t')\label{eq:xLT} \\
\Delta t &= \gamma (\Delta t' +  u\Delta x' /c^2) \label{eq:tLT},
\end{align}
...
We can consider the limit $u\rightarrow 0$. In that case, we know $\gamma \rightarrow 1$ and $\Delta t = \Delta t'$ (as $u/c \ll 1$) while $\Delta x = \Delta x' + u \Delta t'$, i.e., the Galilean transformations.

\subsection{Matrix notation}
Using the time-first convention, we can write the Lorentz transformation in matrix notation as
\be
\begin{pmatrix}
c\Delta t' \\
\Delta x'
...
\end{pmatrix},
\ee
with $\beta = u/c$.  Note that I have considered the inverse transformations (going from frame $S$ to $S'$). In that case we pick up a minus sign in the boost speed, since from the perspective of an observer at rest in frame $S'$, anyone comoving\footnote{I will often use comoving, which often is short-hand for stating that an observer is at rest with respect to some coordinate frame, and moving with (=comoving) the frame as observed from any other inertial reference frame.} with frame $S$ is now moving in the opposite direction.
...
If two events (at e.g., coordinates $(t_1',x_1')$, $(t_2',x_2'))$ occur simultaneously in frame $S'$, we know $(\Delta t',\Delta x') = (0,\Delta x')$, i.e. $t_1'=t_2'$. Using Eq.~\eqref{eq:tLT}, we find that in $S$, $\Delta t= \gamma u \Delta x'/c^2$. Unless $\Delta x' = 0$, the events therefore do not happen simultaneously in frame $S$.
...
\begin{tcolorbox}[colback=blue!5!white]
Frame $S'$ moves to the right with $\beta=u/c=3/5$ relative to frame $S$. In units with $c=1$, an event $P$ has coordinates
\[
(t,x)=(10,4)
\]
in $S$.
