...
Consider a frame at rest $S$ and another frame $S'$ moving with velocity $\beta$.
\begin{tcolorbox}[colback=blue!5!white]
\vspace{-1.3em}
\begin{align}
\Delta x' &= \Delta x - \beta \Delta t \\
\Delta t' &= \Delta t, \\
v' &= v - \beta
\end{align}
\end{tcolorbox}
\noindent 
where $v$ is the velocity of an object in frame $S$ and $v'$ in frame $S'$. Question: Why is there a minus sign?  \\
...
\noindent  
\textbf{These two assumptions contradict the Newtonian view of absolute space and time.} It is `special' because it only concerns non-accelerating frames. This means we can not include gravity for the moment (Diederik might discuss this at the very end if there is time).  


\section{Fundamental effects}
\subsection{Relativity of Simultaneity}\label{sec:Rsim}
Two laser hits both ends of a moving train (receivers). 


\begin{itemize}
    \item \textbf{In frame (S') moving w train:} Light source midway between two receivers. From the observer's point of view in this frame, the light will hit the receivers at the same time. 


    \item \textbf{In ground frame (S'):} The train is moving to the right with speed $v$. Hence, the receiver is moving to the right. As it takes a finite time for the light to travel between the source and the receiver, during this time, the receiver has moved closer. Since the speed of light is constant independent of frame, the relative speed is $v+c$. For the receiver on the right, it is $c-v$. As a result, the moment the light hits the receiver is not the same in this frame. 
\end{itemize}

Note that what we describe here refers to the `observed' time and place, i.e., as measured with a ruler and a clock in a reference frame. The actual observers, who are typically located in the frame, will see things as information reaches their sensors (eyes in our case, but could also be some apparatus). Seeing is therefore not the same as observing. In Moore, this is something that is discussed at some length, but Morin spends only a footnote on this distinction. We might come back to this when we discuss spacetime diagrams (where we can use the so-called radar method to make this distinction clearer).  
...
\item {\bf Observing:} 
What a clock positioned at the event synchronised with the frame of the observer reads off
\end{itemize}
\end{tcolorbox}
...
\begin{tcolorbox}[colback=blue!5!white]
Two events are recorded in frame $S$ using units with $c=1$:
\[
A=(t,x)=(0,0), \qquad B=(5,3).
\]
\begin{enumerate}
    \item Draw the two events on a spacetime diagram and add the light cone from $A$.
    \boardanswer{$A$ is at the origin and $B$ is at $t=5$, $x=3$. Since $|x|<t$, $B$ lies inside the future light cone of $A$.}
    \item Decide whether the separation is timelike, spacelike, or lightlike.
    \boardanswer{$\Delta s^2=\Delta t^2-\Delta x^2=25-9=16>0$, so the separation is timelike.}
    \item Is there an inertial frame in which the two events happen at the same place? Is there one in which they happen at the same time? Explain using the diagram.
    \boardanswer{Because the separation is timelike, there is a frame in which the events happen at the same place. There is no inertial frame in which timelike-separated events are simultaneous.}
    \item What is the proper time or proper distance associated with this separation?
    \boardanswer{The proper time is $\Delta\tau=\sqrt{16}=4$ in units with $c=1$. Proper distance is used for spacelike intervals, so it is not the relevant quantity here.}
\end{enumerate}
\end{tcolorbox}
\fi
