...
\item {\bf Friction:} Friction is the resistive force that opposes motion or the tendency of motion between two surfaces in contact. It acts parallel to the surface and opposite the direction of relative motion (or intended motion). Static friction concerns objects that are at rest relative to each other and adjusts as needed up to $|F_s|\leq \mu_s N$, with $\mu_s$ the coefficient of static friction and $N$ the normal force. Once surfaces slide relative to one another, kinetic friction is often modelled as $|F_k|=\mu_kN$, typically with $\mu_k<\mu_s$.
...
\ee
For $0\leq\mu_s<1$, considering $\tan \theta \geq 1$, we find
\be
...
Newton's laws are 
\begin{itemize}
\item {\bf First Law:} A body moves with constant velocity unless acted upon by a net force. It identifies inertial frames: frames in which Newton's first law holds. These are the same kind of inertial frames to which special relativity applies.
\item {\bf Second Law:} The time rate of change of the momentum of a body equals the net force acting on the body. In classical mechanics, momentum is ${\bf p}=m{\bf v}$, so for constant mass ${\bf F}=m{\bf a}$, with ${\bf a}\equiv d{\bf v}/dt$. This law holds in inertial frames. It expresses an empirical relation between a particle's motion and the net force on it: applying the same net force to particles of different masses produces accelerations inversely proportional to their masses.
\item {\bf Third Law:} For every force on one body, there is an equal and opposite force on another body. Consider two particles with masses $m_1$ and $m_2$ that interact only with each other. The total rate of change of momentum is
\be
\frac{d{\bf p}_{\rm total}}{dt} = \frac{d{\bf p}_{1}}{dt} + \frac{d{\bf p}_{2}}{dt} = {\bf F}_1 + {\bf F}_2 
\ee
with ${\bf F}_1$ and ${\bf F}_2$ the forces acting on the two masses. Newton's third law gives ${\bf F}_1=-{\bf F}_2$, so the total momentum is conserved. For a closed two-particle system, the two particles exchange equal and opposite momentum.
\end{itemize}
As Morin points out, these laws are of intellectual importance, but we should not attach too much significance to them. They are approximations to more fundamental theories. In special relativity, the instantaneous pairwise form of the third law is not generally adequate: for example, electromagnetic fields can carry momentum. The momentum of the particles alone then need not be conserved, but the total momentum of particles and fields is conserved. For a particle, the relativistic momentum is ${\bf p}=\gamma m{\bf v}$, and energy and momentum are naturally combined into a 4-vector.
...
\ee
Defining the integration constant $E=\frac{1}{2} m v_0^2$, we then write
\be
...
\ee
then, in one dimension, any force that depends only on position is conservative. In more than one dimension, position dependence alone is not sufficient: the work must also be path independent. Only for a conservative force can we assign a single-valued potential energy to each position. The gravitational force is an example of a conservative force, also in three dimensions.
\\
...
\noindent
Let us now consider forces in three dimensions. We again consider forces that depend only on position, i.e., ${\bf F }={\bf F }({\bf r})$. This is necessary but not sufficient for a force to be conservative. We can write
\be
...
Now let's come back to conservative forces. In a simply connected region of three-dimensional space, a force is conservative if and only if its curl vanishes: $\nabla \times {\bf F}={\bf 0}$. Then the work is path independent and the potential is uniquely defined (up to an additive constant). We can then write down the differential form:
\be
dV = -{\bf F}({\bf r}) \cdot d{\bf r} \equiv - (F_x dx + F_y dy + F_z dz).
\ee
...
and hence:
\be
F_X=- \frac{\partial V}{\partial X} \Rightarrow {\bf F}({\bf r}) = - \nabla V({\bf r}),
\ee
...
\be
\Delta {\bf p}_i = \int \left(\sum_j {\bf F}_{ij}\right) dt.
\ee
...
\ee
For an isolated system, the center of mass does not move with respect to the center-of-mass frame. We can therefore choose the center of mass to be the origin of that frame. If we take two derivatives with respect to time of the center-of-mass position vector and multiply by $M$, we find
\be
...
In classical dynamics for one-dimensional elastic scatterings, the relative velocity of the two particles after the collision is the negative of the relative velocity before the collision. This can be directly shown using Eqs.~\eqref{eq:conservation}, provided the collision is nontrivial ($v_m^i\neq v_m^f$). We can rearrange those to find
\be
